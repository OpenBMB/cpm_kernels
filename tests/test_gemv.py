...
class TestGemv(unittest.TestCase):
    ...
    def test_gemv_fp16_transpose_light(self):
        with torch.cuda.device(2):
            for _ in range(10):
                BATCH = 16
                N = 128
        ...
        with torch.cuda.device(2):
            for _ in range(5):
                ...
                for i, vec_0 in enumerate(vecs):
                    ...
                    self.assertTrue(torch.isclose(out_0, ans_0, 1e-3, 1e-3).all())
    
    def test_gemv_logits_light(self):
        with torch.cuda.device(2):
            for _ in range(5):
                BATCH = 16
                N = 22222
